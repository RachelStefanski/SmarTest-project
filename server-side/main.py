...
def analyze_question(question_text: str):
    ...
    question_requirements_dicts = question_analysis_no.extract_from_dependencies(question_text)
...
def create_exam():
    ...
    teacher_answer.units_by_category = {category: [] for category in requirements}  # נניח מילוי בהמשך
    question_score = 10
    ...
    student_answer.answer.units_by_category = {category: [] for category in requirements}  # כמו אצל המורה
    ...
    # categories_matrix = [[], []]
    score = answer_processing.calculating_score(teacher_units, student_units, requirements, question_score)
